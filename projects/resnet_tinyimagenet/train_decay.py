...
trainGen = HDF5DatasetGenerator(config.TRAIN_HDF5, 64, aug=aug, preprocessors=[sp, mp, iap], 
...
model = ResNet.build(64, 64, 3, config.NUM_CLASSES, (3, 4, 6), (64, 128, 256, 512), reg=0.0005, 
					dataset="tiny_imagenet")
					
opt = SGD(lr=INIT_LR, momentum=0.9) model.compile(loss="categorical_crossentropy", optimizer=opt, 
			metrics=["accuracy"])
...
model.fit_generator(trainGen.generator(), steps_per_epoch=trainGen.numImages // 64, 	
					...
					epochs=NUM_EPOCHS, max_queue_size=64 * 2, callbacks=callbacks, verbose=1)
